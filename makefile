...
# coq_makefile -f _CoqProject -o makefile ListUtils.v Probabilities.v RatExpr.v Rome.v SPL.v State.v 
#
...
VFILES:=SPL.v\
  ...
  RatExpr.v\
  ListUtils.v\
  TransitionMatrix/Definitions.v\
  TransitionMatrix/Real.v\
  TransitionMatrix/Parametric.v\
  Markov/DTMC.v\
  Markov/PMC.v\
  ReliabilityModels/Definitions.v\
  ReliabilityModels/Annotative.v\
  Rome.v\
  ListUtils.v\
  Probabilities.v\
  RatExpr.v\
...
install:
	cd "." && for i in $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/Rome/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/Rome/$$i; \
	done

install-doc:
	install -d "$(DSTROOT)"$(COQDOCINSTALL)/Rome/html
	for i in html/*; do \
	 install -m 0644 $$i "$(DSTROOT)"$(COQDOCINSTALL)/Rome/$$i;\
	done

uninstall_me.sh: makefile
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Rome && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Rome" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL)/Rome \\\n' >> "$@"
	printf '&& rm -f $(shell find "html" -maxdepth 1 -and -type f -print)\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL) && find Rome/html -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	chmod +x $@
